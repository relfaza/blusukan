...
export async function DELETE(request: Request) {
  ...
  const existing = await prisma.penginapan.findUnique({ where: { id }, select: { id: true } });
  if (!existing) return NextResponse.json({ message: "Data penginapan tidak ditemukan." }, { status: 404 });
