...
const TOP_BADGE: Record<number, { label: string; bg: string; color: string }> = {
  1: { label: "⭐ Rating Terbaik", bg: "#fef3e7", color: "#805533" },
  2: { label: "🥈 Rating Tinggi", bg: "var(--blusukan-primary-container)", color: "var(--blusukan-primary)" },
  3: { label: "🥉 Rating Tinggi", bg: "var(--blusukan-primary-container)", color: "var(--blusukan-primary)" },
};
...
export default async function PeringkatRatingPage() {
    ...
    <div className="min-h-screen" style={{ background: "var(--blusukan-surface)", fontFamily: "Inter, sans-serif" }}>
      <div className="max-w-4xl mx-auto px-4 lg:px-8 py-10">
        ...
        <div className="flex items-center gap-2 mb-1">
          <Star size={22} style={{ color: "var(--blusukan-primary)" }} />
          <h1
            className="text-2xl font-bold"
            style={{ fontFamily: "Montserrat, sans-serif", color: "var(--blusukan-on-surface)" }}
          >
            Peringkat Rating Tertinggi
          </h1>
        </div>
        <p className="text-sm mb-8" style={{ color: "var(--blusukan-on-surface-variant)" }}>
          Diurutkan dari rata-rata rating tertinggi
        </p>
        ...
        {peringkat.length === 0 ? (
          ...
          >
            <Star size={40} style={{ color: "var(--blusukan-outline)" }} className="mb-3" />
            <p className="text-sm font-medium" style={{ color: "var(--blusukan-on-surface-variant)" }}>
              Belum ada destinasi dengan ulasan
            </p>
          </div>
        ) : (
