...
export function buildBucketsForPeriode(periode: "harian" | "mingguan" | "bulanan", now: Date): Bucket[] {
  return periode === "harian" ? buildHarianBuckets(now) : periode === "mingguan" ? buildMingguanBuckets(now) : buildBulananBuckets(now);
}
